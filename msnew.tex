...
identify statistically significant peaks in each FT. We adopt the criterion that a peak must have 
...
as it is impractical to assume that all ``real'' frequencies are removed when determining
the noise level. This is certainly true for GD358, where amplitude modulation is present, and the peaks above $\approx$ 2500 $\mu$Hz\ 
...
Our standard frequency selection procedure involves identifying the largest amplitude resolved peak in the FT,
fitting a sinusoid with that frequency to the data set, subtracting the fit from the light curve, 
