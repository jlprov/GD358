...
\citet{BischoffKim14}. For example, the even period spacing observed in GD358's 
...
\input{gd358fit.tex} % label is gd358freq
...
%If we now look for the models that have $\Delta P_{\rm calc} = \Delta P_{\rm obs} = 39.08$ s 
%and plot their mass and effective temperature, we do recover the expected diagonal trend 
%(Fig \ref{ffit}). We can use the asymptotic period spacing to calibrate our models. Before 
%we seek the detailed chemical make-up by performing period-by-period matching, we can 
%test whether the bulk properties of our models are consistent with our observations of the 
%stars. With spectroscopy, we can obtain a measurement of the effective temperature, and 
%with the addition of a mass-radius relation, the star mass. For GD 358, we use results 
%quoted in XXX and XXX. Both (all X) values are included on Fig. \ref{f3}. We see that our 
%trend of best match between the calculated and the observed average period spacings goes 
%right through the data points. GD 358 is the first DBV for which we are able perform this 
%test. Our models have bulk properties that appear consistent with at least one real star. 
%We confidently move on to our period-by-period fitting.
